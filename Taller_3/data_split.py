...
def split_df(filename):
    ...
    # Assign numerical identifiers to species and genera for evaluation
    #species
    unique_species = bioscan_5M[bioscan_5M["species_name"].notna()]["species_name"].unique()
    sorted_species = sorted(unique_species)
    species_to_index = {species: index for index, species in enumerate(sorted_species)}
    bioscan_5M['species_index'] = bioscan_5M['species_name'].map(species_to_index)
    
    #genera
    unique_genus = bioscan_5M[bioscan_5M["genus_name"].notna()]["genus_name"].unique()
    sorted_genus = sorted(unique_genus)
    genus_to_index = {genus: index for index, genus in enumerate(sorted_genus)}
    bioscan_5M['genus_index'] = bioscan_5M['genus_name'].map(genus_to_index)
